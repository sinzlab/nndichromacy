...
class tricubic:
    def __init__(self, field, *args, **kwargs):
        ...
        )  # Machine precision of floating point number
        ...
        if self.inputfield.shape[1] == 4:
            if not "quiet" in args:
                print(
                    "--- Scalar field, ignoring switches, interpolating for magnitude and gradient --- \n"
                )
            ...
            if "mode" in kwargs:
                if kwargs["mode"] == "vector":
                    if not "quiet" in args:
                        print(
                            "--- Vector field, interpolating for vector components --- \n"
                        )
                    ...
                    self.alphax[:, -1] = self.alphay[:, -1] = self.alphaz[
                        :, -1
                    ] = np.nan
                    ...
                    if not "quiet" in args:
                        print(
                            "--- Vector field, interpolating for magnitude and gradient --- \n"
                        )
                    self.Query = self.Query2
                    self.sQuery = self.sQuery2
                    self.rQuery = self.rQuery2
                    ...
                    if not "quiet" in args:
                        print(
                            "--- Vector field, interpolating vector components plus magnitude and gradient --- \n"
                        )
                    ...
                    if not "quiet" in args:
                        print(
                            "--- Vector field, invalid option, defaulting to interpolating for vector components --- \n"
                        )
                    self.Query = self.Query1
                    self.sQuery = self.sQuery1
                    self.rQuery = self.rQuery1
                ...
                if not "quiet" in args:
                    print(
                        "--- Vector field, no option selected, defaulting to interpolating for vector components --- \n"
                    )
                self.Query = self.Query1
                self.sQuery = self.sQuery1
                self.rQuery = self.rQuery1
    ...
    def makeAMatrix(self):
        ...
        for i in range(64):
            ...
            for k in range(8):
                ...
                B[4 * 8 + k, i] = (
                    ex * x ** (abs(ex - 1)) * ey * y ** (abs(ey - 1)) * z ** ez
                )
                B[5 * 8 + k, i] = (
                    ex * x ** (abs(ex - 1)) * y ** ey * ez * z ** (abs(ez - 1))
                )
                B[6 * 8 + k, i] = (
                    x ** ex * ey * y ** (abs(ey - 1)) * ez * z ** (abs(ez - 1))
                )
                B[7 * 8 + k, i] = (
                    ex
                    * x ** (abs(ex - 1))
    ...
    def sQuery1(self, query):
        ...
        if (
            query[0] < self.xIntMin
            or query[0] > self.xIntMax
            or query[1] < self.yIntMin
            or query[1] > self.yIntMax
            or query[2] < self.zIntMin
            or query[2] > self.zIntMax
        ):
            return np.nan
        else:
            ...
            self.queryInd = (
                ix + iy * (self.nPos[0]) + iz * (self.nPos[0]) * (self.nPos[1])
            )
    ...
    def sQuery2(self, query):
        ...
        if (
            ...
            grad = np.array(
                [
                    np.dot(self.alphan[:, self.queryInd], xx * y * z) / self.hx,
                    np.dot(self.alphan[:, self.queryInd], x * yy * z) / self.hy,
                    np.dot(self.alphan[:, self.queryInd], x * y * zz) / self.hz,
                ]
            )
            ## Magnitude, gradient
            return norm, grad
        ...
        if len(self.queryInds[np.where(self.alphamask[self.queryInds] == 0)[0]]) > 0:
            list(
                map(
                    ...
                    self.queryInds[np.where(self.alphamask[self.queryInds] == 0)[0]],
                )
            )
    ...
    def rQuery2(self, query):
        ...
        grads = np.transpose(
            np.array(
                [
                    ((tn * (xx * y * z)) / self.hx).sum(axis=1),
                    ((tn * (x * yy * z)) / self.hy).sum(axis=1),
                    ((tn * (x * y * zz)) / self.hz).sum(axis=1),
                ]
            )
        )
    ...
    def getFieldParams(self):
        ...
        self.inputfield = self.inputfield[
            self.inputfield[:, 1].argsort(kind="mergesort")
        ]
        self.inputfield = self.inputfield[
            self.inputfield[:, 2].argsort(kind="mergesort")
        ]

        ## Analyse field
        xzero = self.inputfield[
            np.where(self.inputfield[:, 0] == self.inputfield[0, 0])[0]
        ]
        yzero = self.inputfield[
            np.where(self.inputfield[:, 1] == self.inputfield[0, 1])[0]
        ]
        ...
        nPosx = len(
            xaxis
        )  # These give the length of the interpolation volume along each axis
        nPosy = len(yaxis)
        nPosz = len(zaxis)

        self.nPos = np.array(
            [nPosx - 3, nPosy - 3, nPosz - 3]
        )  # number of interpolatable cuboids per axis
        ...
        self.xIntMin = self.inputfield[
            1, 0
        ]  # Minimal value of x that can be interpolated
        self.yIntMin = self.inputfield[nPosx, 1]
        self.zIntMin = self.inputfield[nPosx * nPosy, 2]
        self.xIntMax = self.inputfield[
            -2, 0
        ]  # Maximal value of x that can be interpolated
        ...
        temp = np.array(
            [self.basePointInds + i * nPosx * nPosy for i in range(nPosz - 3)]
        )
        self.basePointInds = np.reshape(temp, (1, len(temp) * len(temp[0])))[0]
...
class quadcubic:
    ...
    def makeAMatrix(self):
        ...
        for i in range(256):
            ...
            for k in range(16):
                ...
                B[1 * 16 + k, i] = (
                    ex * x ** (abs(ex - 1)) * y ** ey * z ** ez * t ** et
                )  # 16:31, df/dx
                B[2 * 16 + k, i] = (
                    x ** ex * ey * y ** (abs(ey - 1)) * z ** ez * t ** et
                )  # 32:47, df/dy
                B[3 * 16 + k, i] = (
                    x ** ex * y ** ey * ez * z ** (abs(ez - 1)) * t ** et
                )  # 48:63, df/dz
                B[4 * 16 + k, i] = (
                    x ** ex * y ** ey * z ** ez * et * t ** (abs(et - 1))
                )  # 64:79, df/dt
    ...
    def sQuery1(self, query):
        ...
        if (
            ...
            or query[2] < self.zIntMin
            or query[2] > self.zIntMax
            or query[3] < self.tIntMin
            or query[3] > self.tIntMax
        ):
            return np.nan
        else:
            ### How many cuboids in is query point
            ...
            self.queryInd = (
                iix
                + iiy * (self.nPosx - 3)
                + iiz * (self.nPosx - 3) * (self.nPosy - 3)
                + iit * (self.nPosx - 3) * (self.nPosy - 3) * (self.nPosz - 3)
            )
            self.queryInd = self.queryInd.astype(int)
            ### Calculate alpha for cuboid if it doesn't exist
            if self.alphamask[self.queryInd] == 0:
    ...
    def sQuery2(self, query):
        ...
        if (
            ...
            grads = np.array(
                [
                    np.dot(self.alphan[:, self.queryInd], xx * y * z * t) / self.hx,
                    np.dot(self.alphan[:, self.queryInd], x * yy * z * t) / self.hy,
                    np.dot(self.alphan[:, self.queryInd], x * y * zz * t) / self.hz,
                    np.dot(self.alphan[:, self.queryInd], x * y * z * tt) / self.ht,
                ]
            )
            return norm, grads
    ...
    def rQuery2(self, query):
        ...
        grads = np.transpose(
            np.array(
                [
                    ((tn * (xx * y * z * t)) / self.hx).sum(axis=1),
                    ((tn * (x * yy * z * t)) / self.hy).sum(axis=1),
                    ((tn * (x * y * zz * t)) / self.hz).sum(axis=1),
                    ((tn * (x * y * z * tt)) / self.ht).sum(axis=1),
                ]
            )
        )
        return norms, grads
    ...
    def getFieldParams(self):
        ...
        self.tIntMin = self.inputfield[
            self.nPosx * self.nPosy * self.nPosz, 3
        ]  # right I think
        self.xIntMax = self.inputfield[
            -2, 0
        ]  # Maximal value of x that can be interpolated
        ...
        self.tIntMax = self.inputfield[
            (-2 * self.nPosx * self.nPosy * self.nPosz), 3
        ]  # OK then!
        ...
        temp = np.array(
            [self.basePointInds + i * self.nPosx for i in range(self.nPosy - 3)]
        )
        self.basePointInds = np.reshape(temp, (1, len(temp) * len(temp[0])))[0]
        temp = np.array(
            [
                self.basePointInds + i * self.nPosx * self.nPosy
                for i in range(self.nPosz - 3)
            ]
        )
        self.basePointInds = np.reshape(temp, (1, len(temp) * len(temp[0])))[0]
        temp = np.array(
            [
                self.basePointInds + i * self.nPosx * self.nPosy * self.nPosz
                for i in range(self.nPost - 3)
            ]
        )
        self.basePointInds = np.reshape(temp, (1, len(temp) * len(temp[0])))[0]
