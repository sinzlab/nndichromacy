...
from .utility import *
from .shifters import MLPShifter, StaticAffine2dShifter, TrialIDShifter
from .encoders import Encoder, GeneralEncoder, EncoderLearnedShifter
from .cores import SE2dCore, TransferLearningCore, GaussianLaplaceCore
from .readouts import (
    ...
    MultipleCenterSurround,
    MultiplePointPyramid2d,
)
...
def se_core_full_gauss_readout(
    ...
    if grid_mean_predictor is not None:
        ...
        if grid_mean_predictor_type == "cortex":
            ...
            if data_info is None:
                ...
                source_grids = {
                    k: v["cell_motor_coordinates"][:, :input_dim]
    ...
    if shifter is True:
        data_keys = (
            list(dataloaders.keys()) if data_info is None else list(data_info.keys())
        )
        if shifter_type == "MLP":
            shifter = MLPShifter(
                data_keys=data_keys,
    ...
    model = Encoder(
        ...
        shifter=shifter,
        linear=linear,
        final_nonlinear_for_linear=final_nonlinear_for_linear,
        n_neurons_dict=n_neurons_dict,
    )
...
def se_core_full_gauss_readout_learned_shifter(
    dataloaders,
    seed,
    hidden_channels=32,
    input_kern=13,  # core args
...
from .utility import purge_state_dict


def simple_core_transfer_w_shifter(
    dataloaders,
    seed,
    transfer_key=dict(),
    ...
    share_grid=False,
    shifter=None,
    shifter_type="embedding",
    input_channels_shifter=1,
    hidden_channels_shifter=10,
    shift_layers=3,
    shifter_bias=False,
    gamma_shifter=0,
    **kwargs,
    ...
    if core_transfer_table != "TrainedModel":
        raise ValueError("Transfer is only possible from TrainedModel table.")
    ...
    for k in dataloaders:
        data_keys.append(k)
        n_trials.append(dataloaders[k].dataset.trial_info.trial_idx.max() + 1)
    ...
    if shifter is True:
        ...
        if shifter_type == "MLP":
            ...
            shifter = TrialIDShifter(data_keys=data_keys, n_trials=n_trials)
    else:
        shifter = None
    model = EncoderLearnedShifter(
        core=core,
...
def full_transfer_w_shifer(
    ...
    **kwargs,
    ...
    model = EncoderLearnedShifter(
        ...
        shifter=shifter,
        shifter_type=shifter_type,
        use_pos_embedding=pos_embedding_shifter,
        d_model=d_model_shifter,
    )
    return model
...
def se_core_center_surround(
    ...
    readout = MultipleCenterSurround(
        core,
        in_shape_dict=in_shapes_dict,
        n_neurons_dict=n_neurons_dict,
        init_mu_range=init_mu_range,
...
def gl_core_full_gauss_readout(
    dataloaders,
    seed,
    hidden_channels=32,
    input_kern=13,  # core args
...
def gl_core_pyramid_readout(
    ...
    readout = MultiplePointPyramid2d(
        core,
        in_shape_dict=in_shapes_dict,
        n_neurons_dict=n_neurons_dict,
        bias=readout_bias,
